...
@IBDesignable
public class AGFaviButton: UIButton {
...
//            line.transform = CATransform3DMakeRotation(CGFloat(M_PI) / 5 * (CGFloat(i) * 2 + 1), 0.0, 0.0, 1.0)
    ...
    fileprivate func addTargets() {
        ...
        // add target
        //===============
        self.addTarget(self, action: #selector(touchDown(_:)), for: UIControl.Event.touchDown)
        self.addTarget(self, action: #selector(touchUpInside(_:)), for: UIControl.Event.touchUpInside)
       
        self.addTarget(self, action: #selector(touchDragExit(_:)), for: UIControl.Event.touchDragExit)
        self.addTarget(self, action: #selector(touchDragEnter(_:)), for: UIControl.Event.touchDragEnter)
        self.addTarget(self, action: #selector(touchCancel(_:)), for: UIControl.Event.touchCancel)
    }
    
    @objc func touchDown(_ sender: AGFaviButton) {
        self.layer.opacity = 0.4
    }
    @objc func touchUpInside(_ sender: AGFaviButton) {
        self.layer.opacity = 1.0
    }
    @objc func touchDragExit(_ sender: AGFaviButton) {
        self.layer.opacity = 1.0
    }
    @objc func touchDragEnter(_ sender: AGFaviButton) {
        self.layer.opacity = 0.4
    }
    @objc func touchCancel(_ sender: AGFaviButton) {
        self.layer.opacity = 1.0
    }
